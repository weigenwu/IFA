...
- `/intensity`：确认 1–8 个通道，设置 Olympus 伪彩、ImageJ 风格显示范围和背景杂色抑制，框选固定边长正方形 ROI，输出强度指标、双通道线扫描及带比例尺的裁切图。
...
4. 强度页可直接输入正方形边长（px 或 µm）；锁定后在框内拖动只改变位置。显示去杂色只影响预览和图片导出，定量扣背景仍单独设置。
