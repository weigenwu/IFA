...
- `/intensity`：勾选 1–8 个通道，设置奥林巴斯风格伪彩、显示黑场和背景杂色抑制，框选固定边长正方形 ROI，输出强度指标、双通道线扫描及带比例尺的裁切图。

## 输入适配

- Olympus FV3000 `.oir`：直接在浏览器本地读取 8/16-bit 原始通道、有效位深、通道名与像素尺寸，无需 ImageJ。Z-stack 自动逐通道生成最大强度投影（MIP）并写入质控记录。
...
- 通道伪彩可在绿色、红色、蓝色、青色、洋红、黄色、橙色、紫色和白色/灰度之间切换；基础色使用 ImageJ / Olympus 纯加色 LUT，预览与裁切默认按每通道的原始最小值–最大值显示。可用无细胞暗区的背景 ROI 按“均值 + 2 SD”抑制显示杂色；伪彩、显示黑场和该抑制均不改变原始像素或计算结果。TD 透射光默认不参与强度分析，但可手动选入。
...
3. 共定位页映射 A/B；强度页勾选 1–8 个通道并调整名称与伪彩。如需去杂色，点图下方“背景 ROI”，在无细胞暗区拖框，再勾选“按背景 ROI 均值 + 2 SD 抑制杂色”。
4. 强度页拖拽正方形裁剪区，可输入统一边长；核对 OIR 像素尺寸并设置比例尺。显示去杂色只影响预览和图片导出，定量扣背景仍单独设置。
